...
class MetricsComputer:
    ...
    def reset(self):
        ...
        self._count_delta = 0
        # Delta calibration: regress true on pred (slope, intercept, R²)
        # Sign accuracy: fraction where sign(pred_delta) == sign(true_delta)
        self._count_sign_correct = 0
    ...
    def update(
        ...
        # Sign accuracy: same sign (treat 0 as positive for consistency)
        sign_pred = torch.sign(delta_pred)
        sign_true = torch.sign(delta_true)
        sign_pred = torch.where(sign_pred == 0, torch.ones_like(sign_pred), sign_pred)
        sign_true = torch.where(sign_true == 0, torch.ones_like(sign_true), sign_true)
        self._count_sign_correct += (m_delta * (sign_pred == sign_true).float()).sum().item()
    ...
    def compute(self) -> Dict[str, float]:
        ...
        if self._count_delta > 0:
            ...
            var_dp = self._sum_delta_pred_sq / n - mean_dp ** 2 + eps
            ...
            if var_dp > eps:
                slope = cov / var_dp
                intercept = mean_dt - slope * mean_dp
                metrics["delta_slope"] = float(slope)
                metrics["delta_intercept"] = float(intercept)
                ss_tot_d = self._sum_delta_true_sq / n - mean_dt ** 2
                # SS_res = E[(true - (intercept + slope*pred))^2]
                ss_res_d = (
                    self._sum_delta_true_sq / n
                    - 2 * intercept * mean_dt
                    - 2 * slope * (self._sum_delta_pred_true / n)
                    + intercept ** 2
                    + 2 * intercept * slope * mean_dp
                    + slope ** 2 * (self._sum_delta_pred_sq / n)
                )
                metrics["delta_r2"] = float(1.0 - ss_res_d / (ss_tot_d + eps)) if ss_tot_d > eps else 0.0
            else:
                metrics["delta_slope"] = float("nan")
                metrics["delta_intercept"] = float("nan")
                metrics["delta_r2"] = float("nan")
            metrics["sign_accuracy"] = self._count_sign_correct / n
        else:
            ...
            metrics["delta_intercept"] = float("nan")
            metrics["delta_r2"] = float("nan")
            metrics["sign_accuracy"] = float("nan")
...
def format_metrics(metrics: Dict[str, float]) -> str:
    ...
    if "delta_slope" in metrics and not np.isnan(metrics.get("delta_slope", nan)):
        lines.append(
            f"Delta calibration - slope: {metrics['delta_slope']:.4f}, "
            f"intercept: {metrics['delta_intercept']:.4f}, "
            f"R²: {metrics.get('delta_r2', nan):.4f}"
        )
    if "sign_accuracy" in metrics and not np.isnan(metrics.get("sign_accuracy", nan)):
        lines.append(f"Sign accuracy: {metrics['sign_accuracy']:.4f}")

...
class SyntheticBenchmark:
    ...
    def run_many_k_benchmark(
        ...
        Returns:
            Nested dict: results[scm_type][k] = metrics dict (delta_correlation, delta_rmse, cf_rmse, ate_mae, ...).
        """
        if scm_types is None:
            scm_types = [
                'linear_gaussian', 'nonlinear_additive', 'multiplicative',
                'heteroskedastic', 'heavy_tailed', 'high_dimensional'
            ]
        out = {}
        for scm_type in scm_types:
            ...
            for k in k_list:
                try:
                    m = self.evaluate_scm(scm_type, n_samples=n_samples, n_interventions=k, seed=seed)
                    out[scm_type][k] = {key: m[key] for key in [
                        'delta_correlation', 'delta_rmse', 'delta_mae', 'cf_rmse', 'ate_mae'
                    ] if key in m}
                except Exception as e:
                    out[scm_type][k] = {'error': str(e)}
        return out

...
class SCMEpisodeDataset(IterableDataset):
    ...
    def _generate_episode(self, episode_idx: int) -> Episode:
        ...
        # Optional: random permutation of non-baseline worlds (reproducible via episode_rng)
        K = len(interventions_list)
        if getattr(self.stage, "shuffle_intervention_order", True) and K > 1:
            perm = episode_rng.permutation(K)
            query_x_worlds = np.concatenate(
                [query_x_worlds[0:1], query_x_worlds[1 + perm]], axis=0
            )
            query_y_worlds = np.concatenate(
                [query_y_worlds[0:1], query_y_worlds[1 + perm]], axis=0
            )

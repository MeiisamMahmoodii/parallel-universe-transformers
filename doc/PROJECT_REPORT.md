...
2. An **IHDP-style** benchmark is provided (single treatment, W=2); results are comparable to the literature but the model is run in a reduced setup (see [EVALUATION.md](EVALUATION.md) §4b).
3. Harder curriculum stages show modest delta_correlation (~0.04–0.07).
4. Current comparison is vs mean stub and an outcome (ridge) baseline; Do-PFN can be added via the same protocol interface.
5. **Metric limitations:** delta_correlation alone does not guarantee good calibration or scale; reporting slope/intercept of (true deltas vs predicted deltas) and sign accuracy would strengthen claims.
...
5. **Intervention-order randomization** is implemented: each episode permutes the non-baseline worlds (1..K) so the model does not rely on world index; toggle via `shuffle_intervention_order` in the curriculum stage.

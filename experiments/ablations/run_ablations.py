...
def run_world_count_ablation():
    """Ablation: Impact of number of worlds during training."""
    print("\n" + "="*60)
    print("ABLATION: World Count")
    print("="*60 + "\n")
    
    # Note: This requires modifying the curriculum config
    # For simplicity, we'll document the approach
    
    print("World count ablation requires modifying CurriculumConfig.")
    print("Recommended world counts to test: W=3, W=5, W=9, W=17")
    print("Expected finding: More worlds → better generalization but slower training")
...
def main():
    ...
    parser.add_argument(
        '--ablation',
        choices=['cross_attention', 'delta_loss', 'world_count', 'all'],
        default='all',
    ...
    if args.ablation in ['world_count', 'all']:
        run_world_count_ablation()
